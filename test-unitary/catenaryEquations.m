...
Aeq = [];
beq = [];
p_fmc = fmincon(@(p_hat)fcout1(Pcat2d_samp(1,:),Pcat2d_samp(2,:),rlen,hmax,p_hat,cam_pc),pinit,A,b,Aeq,beq,lb,ub); % minimize error on Y axis
...
Pcat3d_fmc  = catenary3D(rlen,hmax,p_fmc,cam_pc); % estimation from fmincon
Pcat3d_fmc_plt = Pcat3d_fmc;
for i=1:length(Pcat3d_plt)
    Pcat3d_fmc_plt(:,i) = rotm*Pcat3d_fmc(:,i);
end
Pcat3d_gna  = catenary3D(rlen,hmax,p_gna,cam_pc); % estimation from gna
...
Pcat2d_fmc = catenaryProjection(rlen,hmax,p_fmc, Pcat3d_fmc(1,:), Pcat3d_fmc(2,:), Pcat3d_fmc(3,:),cam_pc); % estimation from fmincon
Pcat2d_tst = catenaryProjection(rlen,hmax,s,Pcat2d_samp(1,:),cam_pc);
Pcat2d_gna = catenaryProjection(rlen,hmax,p_gna,cam_pc); % estimation from gna
figure();
...
l=legend('real','observed','Fmincon','Gauss-Newton','Validation');l.Location='best';
